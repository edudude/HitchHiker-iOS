...
class HomeVC: UIViewController, Alertable {
    ...
    override func viewDidLoad() {
        ...
        DataService.instance.REF_DRIVERS.observe(.value) { (snapshot) in
            ...
            DataService.instance.passengerIsOnTrip(passengerKey: self.currentUserId!, handler: { (isOnTrip, driverKey, tripKey) in
                if isOnTrip == true {
                    self.zoom(toFitAnnontationsFromMapview: self.mapView, forActiveTripWithDriver: true, withKey: driverKey)
                }
            })
        }
        ...
        DataService.instance.REF_DRIVERS.observeSingleEvent(of: .value) { (snapshot) in
            if let driverSnapshot = snapshot.children.allObjects as? [DataSnapshot] {
                for driver in driverSnapshot {
                    if driver.hasChild(USER_IS_DRIVER) { // check this again, probably not needed
    ...
    func buttonSelector(forAction action: ButtonAction) {
        ...
        case .getDirectionsToDestination:
            DataService.instance.driverIsOnTrip(driverKey: self.currentUserId!, handler: { (isOnTrip, driverKey, tripKey) in
                if isOnTrip == true {
                    DataService.instance.REF_TRIPS.child(tripKey!).child(USER_DESTINATION_COORDINATE).observe(.value
                        , with: { (snapshot) in
...
extension HomeVC: MKMapViewDelegate {
    ...
    func zoom(toFitAnnontationsFromMapview mapView: MKMapView, forActiveTripWithDriver: Bool, withKey key: String?) {
